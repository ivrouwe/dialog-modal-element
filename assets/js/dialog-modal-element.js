class DialogModalElement extends HTMLElement {
    constructor() {
		...
		let div = CustomElementHelpers.createElementFromString(`<div is="dialog-modal-element"></div>`),
			...
			closeControl = CustomElementHelpers.createElementFromString(`<button type="button" id="dialog-modal--${idInteger}__close-control" class="dialog__close-control">Close Dialog</button>`),
		...
		if (0 !== startingHeadingLevelOffset) {
			dialogHeadings.forEach(heading => {
				...
				changeTag(heading, `h${newHeadingLevel}`, true);
			});
		}
